  ...
  useEffect(() => {
    ...
    const unsubs = workoutDays.map((day) => {
      return onSnapshot(doc(db, "customers", user.uid, "workouts", day.dateStr), (docSnap) => {
        setWorkouts(prev => ({ 
          ...prev, 
          [day.dateStr]: docSnap.exists() ? docSnap.data().exercises || [] : [] 
        }));
      });
    });
        ...
        <View style={styles.workoutCardFixed}>
          {dayExercises.length > 0 ? (
            ...
            <View style={styles.emptyContent}>
              ...
              >
                <Text style={styles.planButtonText}>Plan Workout</Text>
              </TouchableOpacity>
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  // FIXED: Missing exercisePreviewArea style added
  exercisePreviewArea: { 
    flex: 1, 
    paddingTop: 5 
  },
  ...
  startWorkoutBtnText: { color: '#FFF', fontWeight: 'bold' },
